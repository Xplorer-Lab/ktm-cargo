...
export const calculateFinancials = (shipments = [], shoppingOrders = [], expenses = []) => {
  ...
  const totalVendorPaid = shoppingOrders.reduce((sum, o) => {
    if (String(o.vendor_payment_status || '').toLowerCase() === 'paid') {
      return (
        sum +
        (Number(o.vendor_cost) ||
          Number(o.actual_product_cost) ||
          Number(o.estimated_product_cost) ||
          0)
      );
    }
    return sum;
...
export const useDashboardData = () => {
  ...
  const shipmentCategories = useMemo(() => categorizeShipments(shipments), [shipments]);
