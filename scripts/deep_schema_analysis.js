...
try {
  ...
  const shipmentSchemaMatch = content.match(
    /export const shipmentSchema = z\.object\(\{([^}]+(?:\{[^}]*\}[^}]*)*)\}\)/s
  );
...
for (const table of tables) {
  ...
  const extra = [...actual].filter(
    (f) => !expected.has(f) && !['id', 'created_at', 'updated_at', 'created_date'].includes(f)
  );
...
for (const [table, info] of Object.entries(mismatches)) {
  if (info.missing.length > 0) {
    ...
    for (const column of info.missing) {
      ...
      if (column.includes('count') || column.includes('quantity') || column.includes('stock'))
        dataType = 'INTEGER';
      if (column.includes('is_') || column.includes('_opted') || column.includes('_active'))
