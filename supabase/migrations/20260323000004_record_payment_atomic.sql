-- P0-3a: record_payment_atomic
-- Locks the invoice row with FOR UPDATE before updating balances so that
-- concurrent payments cannot read stale amount_paid / balance_due.
-- Called by InvoiceService.recordPayment() — this RPC must exist in the DB.
...
BEGIN
  ...
  -- Acquire row-level lock — blocks concurrent payment/refund on same invoice
